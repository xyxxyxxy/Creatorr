...
| **Root folder** / **Quality profile** | Absolute path (+ optional display name, optional retention TTL) / named yt-dlp `--format` selector plus optional maturity delays and optional SponsorBlock mark/remove/reencode/info-cards. Format selector applies to **video** delivery only - **audio** delivery ignores it and always uses the fixed `ba/bestaudio/b` ladder. Remux to MKV (video) or MKA (audio). |
