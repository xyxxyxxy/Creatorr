...
| **acquired_via** | How packed media was obtained: `source` (live download), `archive` (Web Archive pack), `import` (disk import). Durable; maturity media/sidecar skips `archive`. |
...
| `videos` | Indexed video: source_id, title, status, season/episode, `source_url`, `acquired_via` (`source`\|`archive`\|`import`), `acquired_at` / `sidecars_acquired_at` (set on pack; sidecar maturity updates latter only), duration/resolution/download columns, `media_type`; `upload_date` stored as RFC3339 UTC |
...
| `schema_version` | Single-row integer; Open applies `schema.sql` then stepwise migrations (`internal/db/migrate.go`). v2: add `sources.full_scan_limit`, drop `sources.scan_cutoff`. v3: clear `wanted_source_error` status + hold history rows. v4: add `videos.acquired_via` and backfill (`import_src` → `import`, else `source`) |
