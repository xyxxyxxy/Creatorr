...
| **Notification** | In-app row for every notify event (`notifications`), delivered by the fixed Creatorr channel. **Alert** events (`cookie_invalid` / `rate_limited` / `ytdlp_failed` / `verify_failed` / `file_sync_issues`) stay unread until History open (marks all), detail open, or mark-read and require `task_id`. Apprise success sets `external_ok` only (does not clear unread). **Info** events (`download_digest`, `live_skipped`, `archive_fallback`, `episode_rename_queued`) are stored read; digest has no `task_id`, live skip / archive pack / episode-rename-queued set `task_id` to link the finished or queued task. Apprise channels are optional fan-out. |
...
- `verify_failed` - packed media failed post-pack null-decode verify; **file kept**. **Want** → `wanted`; **Queue download** re-downloads. File sync treats like `downloaded` (path gone → `missing`; path back → `downloaded`). Not eligible for media maturity while in this status.
...
**No video monitored flag.** Eligibility is status only (`wanted` vs `ignored` / `deleted` / `missing` / errors / archive wait). Download-wanted for status `wanted` when series monitored ∧ domain active. **Queue download** also accepts ignore/deleted/missing/error/`verify_failed`/`wanted_archive` (cancels archive-lane download, sets `wanted`), bypasses the queue cap, and may run when series is unmonitored (domain active still required). **Want** sets `ignored` / `deleted` / `missing` / `verify_failed` → `wanted` without enqueueing. Source **Retry** clears `wanted_download_error` and `wanted_archive` → `wanted` (cancels archive-lane tasks).
...
- post-pack media verify ok (`verified`) or fail (`verify_failed` + status `verify_failed`; file kept; `MediaVerifyFailed`).
