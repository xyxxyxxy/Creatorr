...
- Integrity check ok (`integrity_checked`; legacy History may still say `verified`) or fail (`integrity_check_failed` + status `integrity_check_failed`; file kept; `IntegrityCheckFailed`). Video History Event column shows the task kind (`integrity_check` / `integrity_check_initial`); Message is the result (e.g. `Integrity check ok`).
