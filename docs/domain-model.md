...
| `schema_version` | Single-row integer; Open applies `schema.sql` then stepwise migrations (`internal/db/migrate.go`). Current target **v13**. v2: add `sources.full_scan_limit`, drop `sources.scan_cutoff`. v3: clear `wanted_source_error` status + hold history rows. v4: add `videos.acquired_via` and backfill (`import_src` → `import`, else `source`). v5: per-root `root_folders.episode_format` (copy legacy `settings.episode_format`, drop key). v6: drop `auto_ignore_media_types` from series/sources. v7: drop `videos.tool`. v8: make `acquired_via` nullable; clear via when never acquired (v4 source-for-everyone was migration-only). v9: bump exact old default episode format to `E{episode:04}` padded default. v10: add `files.content_hash`; clear NFO `size_bytes`; rename integrity status/kind/notify from legacy `verify_*` / media-verify names. v11: add `tasks.origin` + `parent_task_id`; backfill all origin=`manual`; strip legacy payload/detail `trigger` keys. v12: rename leftover `video_history.event` (`verified`→`integrity_checked`, `verify_failed`→`integrity_check_failed`) and rewrite legacy kind strings in history detail JSON. v13: add `tasks.logs` (progress lines kept on failed Finish) |
