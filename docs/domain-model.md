...
| **Notification** | In-app row for every notify event (`notifications`), delivered by the fixed Creatorr channel. **Alert** events (`cookie_invalid` / `rate_limited` / `ytdlp_failed` / `integrity_check_failed` / `file_sync_issues`) stay unread until History open (marks all), detail open, or mark-read and require `task_id`. Apprise success sets `external_ok` only (does not clear unread). **Info** events (`download_digest`, `live_skipped`, `archive_fallback`) are stored read; digest has no `task_id`, live skip / archive pack set `task_id` to link the finished task. Apprise channels are optional fan-out. |
