...
| `schema_version` | Single-row integer; Open applies `schema.sql` then stepwise migrations (`internal/db/migrate.go`). v2: add `sources.full_scan_limit`, drop `sources.scan_cutoff`. v3: clear `wanted_source_error` status + hold history rows. v4: add `videos.acquired_via` and backfill (`import_src` → `import`, else `source`). v7: drop `videos.tool`. v8: make `acquired_via` nullable; clear via when never acquired (v4 source-for-everyone was migration-only) |
