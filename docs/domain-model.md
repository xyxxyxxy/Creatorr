...
| **Episode** | Library / media-server view of a packed **video**: season/episode numbers, episode filename stem, episode NFO (`episodedetails`). Created at **pack** from a video; the series' root folder `episode_format` (relative under the series folder; tokens include `{year}`, `{title:N}`, `{episode}`, `{month}`, `{day}`, `{date}`, `{domain}`, optional `{series}`) applies to on-disk episodes, not to the DB video title shown in the UI. Series folder is always `SeriesDir` (sanitized title, max 100 runes). |
...
| **Notification** | In-app row for every notify event (`notifications`), delivered by the fixed Creatorr channel. **Alert** events (`cookie_invalid` / `rate_limited` / `ytdlp_failed` / `verify_failed` / `file_sync_issues`) stay unread until History open (marks all), detail open, or mark-read and require `task_id`. Apprise success sets `external_ok` only (does not clear unread). **Info** events (`download_digest`, `live_skipped`, `archive_fallback`) are stored read; digest has no `task_id`, live skip and archive pack set `task_id` to link the finished task. Apprise channels are optional fan-out. |
