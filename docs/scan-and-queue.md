...
| `integrity_check_initial` | After successful archive pack when profile gate says so; import confirm `verify` when File integrity on | System lane: ffmpeg null-decode (`-xerror`) of library media with `-progress`, plus hash/NFO when File integrity on. Success → history `integrity_checked`. Fail → keep files, status `integrity_check_failed`, notify `integrity_check_failed`, code `IntegrityCheckFailed`. Cancel leaves `downloaded`. New pack cancels prior verify (superseded). |
