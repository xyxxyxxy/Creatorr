...
| `download_rate_limit` | `10M` | Passed to yt-dlp as `--limit-rate` for archive download, scan/list, and **`cache_beginning`**. UI: number + unit join (`K`/`M`/`G`, or Unlimited → `off`). `off` / `0` / `none` = unlimited. Not used for `stream_play` or **interactive** metadata prefetch (`prefetch_*`). |
