...
| `verify_all_media` | `system` | One pending/running; Settings → Maintenance Run → Verify all downloaded videos; optional payload `series_ids` / `video_ids`; resumable cursor |
...
| **Verify all media** | Settings → Maintenance Run → **Verify all downloaded videos** | System task `verify_all_media` (one pending/running; resumable cursor). Optional Maintenance scope `series_ids` / `video_ids`. Null-decodes packed `downloaded` / `verify_failed` media (same ffmpeg path as `media_verify`). Success → history `verified` + status `downloaded`. Fail → keep files, `verify_failed`, notify `verify_failed`. Skips videos with pending/running download / cut / verify. Ignores quality-profile verify gate. |
