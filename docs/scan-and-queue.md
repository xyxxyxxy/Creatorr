...
| `sync_files` | `system` | One pending/running; cron enqueues at higher priority than Apply |
| `retention_delete` | `system` | One pending/running; cron enqueues at higher priority than Apply |
| `rename_episodes` | `system` | One pending/running; Maintenance → Apply episode format |
| `regenerate_nfo` | `system` | One pending/running; Settings → Library → Regenerate NFO; resumable cursors |
| `regenerate_strm` | `system` | One pending/running; Settings → Library → Regenerate strm; resumable cursors |
| `clear_beginning_cache` | `system` | One pending/running; Settings → Library → Clear beginning of stream cache |
| `clear_playback_cache` | `system` | One pending/running; Settings → Library → Clear progressive stream cache |
...
| `prefetch_series_meta` | fetch URL hostname | **Interactive:** ClaimInteractive + concurrent run; ignores soft Pause (and domain busy/cooldown); Finish does not start cooldown; still requires domain active |
| `prefetch_video_meta` | fetch URL hostname | **Interactive:** same as prefetch_series_meta; resolve (+ optional sidecars) into video Metadata modal draft |
| `prefetch_add_series` | fetch URL hostname | **Interactive:** Add series wizard fetch; draft under `cache/add-series/{token}/` (no series row yet); ignores soft Pause; Finish does not start cooldown |
| `stream_play` | stream proxy (live or cache play) | **Interactive occupancy:** `InsertRunning` (not ClaimInteractive); ignores soft Pause; Finish does not start cooldown; **does** count toward `max_parallel_tasks` so ClaimNext waits; Cancel stops live mux; play-time yt-dlp pause codes auto soft-pause like worker |

System maintenance runs **concurrent with hostname** work (worker goroutines after claim). The **`system` lane is always serial** (exactly one running task), hard-coded in claim - Settings `max_parallel_tasks` and `task_cooldown_seconds` do not apply to `system` (no cooldown between system tasks). Tasks page always shows the `system` lane (pinned first) plus a lane for every known host (domains rows and source URL hostnames), including empty queues.
...
| Mode | When | Behavior |
|---|---|---|
| **Full scan** (`mode=full`) | Source `full_scan_done` is false | One task lists the whole feed (uncapped, newest-first), upserts entries **until scan cutoff** (on/before cutoff day are **not** indexed; walk stops), then sets `full_scan_done`. Domain must be active; series monitored **not** required. Status: **scanning** / **queued** while a task runs; **pending** if domain inactive with no task; **incomplete** if domain active but no task queued. |
...
**Kick (series Scan / per-source due):** enqueue tip **Scan** for feed sources with full scan done when series monitored ∧ domain active. Manual Scan is allowed even if the source interval is `never`. **Scheduled Scan cron** (`EnqueueScansDue`): same due check for all scheduled feeds; enqueues tip Scan when full scan done, else full scan (`EnqueueScanSource` mode switch). **Add source / Full scan / API full scan:** always enqueue full scan when domain active (series monitored not required). Domain queue + per-domain delay spaces execution. Interrupted full scans resume via `RequeueStaleRunning` (re-list + idempotent upserts), **Full scan**, or the next due schedule tick.
...
**Mark new videos as ignored** (`index_as_ignored`): when on (**feed** sources only; UI hidden for singles), new videos get status `ignored` instead of `wanted` until manually set to wanted. **Title include / exclude** (`title_regexp_include`, `title_regexp_exclude`): optional Go regexps on **feed** sources (UI hidden for singles). Index only when include matches (if set) and exclude does not match (if set); **exclude wins** when both match (no video row). Tip scan walks past title skips (continue; stop only on known remote id or cutoff). Empty = no filter. Invalid patterns rejected on save. **Auto ignore filters** (`auto_ignore_media_types`): optional on **series** (JSON string array of yt-dlp `media_type` values; Edit series / add-series under Auto ignore filters). Videos are indexed, then marked `ignored` when type is known (create-time if listed; download `--match-filters` or stream `pack_stream` otherwise; before `index_as_ignored` at create). Missing/empty media type is never auto-ignored (type often unknown at list time). Empty list = none. **Scan cutoff** (optional YYYY-MM-DD): full scan and tip Scan stop at the first listing on/before that UTC day; those videos are **not** indexed.

**Tip scan + `download_new_on_scan` (default on, Settings → Scheduler):** after a tip scan indexes new `wanted` videos on a monitored series, enqueue downloads immediately (order = `download_wanted_order`) until that hostname’s **Max download queue** is full. Full scan stays index-only. Leftover wanted (full queue, or from full scan) wait for `download_wanted_cron` / **Download now**.

**Per-domain settings (UI: default + overrides on Settings → Queue):**

| Field | Default | Role |
|---|---|---|
| `task_cooldown_seconds` | `30` | Pause after a task **starts** before another may start on that host domain (still applies with parallel tasks). Interactive kinds (prefetch + `stream_play`) do not start cooldown. Does not apply to the `system` lane. |
...
| `download_rate_limit` | `10M` | Passed to yt-dlp as `--limit-rate`. UI: number + unit join (`K`/`M`/`G`, or Unlimited → `off`). `off` / `0` / `none` = unlimited. |
| `sleep_requests` | `1` | Seconds for yt-dlp pacing: `--sleep-requests`, `--sleep-subtitles`, and `--sleep-interval` (same value). `0` = off. |

Global defaults live on `domains` row **`default`** (non-NULL limit columns). Per-host overrides are other `domains` rows (NULL = inherit `default`). Missing host row = implicitly active + `default` limits. Operator creates/deletes overrides on Settings → Queue (or Tasks lane Edit). Reserved name `default` is not a host override.
...
**History (finished scan task):** one finished scan task with detail, e.g. `Scan: indexed 12 videos (3 new, 1 skipped by title include, 1 skipped by title exclude, 1 ignored by media type, 1 marked new as ignored) [full]` or `[scan]`; `created_ids` / `updated_ids`; `skipped_title_regexp_include` / `skipped_title_regexp_exclude` (`[{remote_id, title}, …]`); `ignored_media_type_ids` / `ignored_index_as_ignored_ids` (subsets of creates). History list shows the task message only (open task detail for video links). Task detail (`/task/{id}`): Detail section lists JSON keys; `created_ids` show id, title, required scan-time state (`wanted`\|`ignored`), and ignore reason when ignored; `skipped_title_regexp_include` / `skipped_title_regexp_exclude` list titles (no video links; ~20 then “and N more”); ignore id arrays render as counts. Not one History row per video.
...
| `download` | Download-wanted cron, Download now, retry, **maturity media** (payload `maturity`) | Handler fetch; when profile **remove** is empty: remux + optional mark-only SB + **pack**. When remove is set: stage under `{CacheDir}/sponsorblock-cut/{videoID}/` and enqueue `sponsorblock_cut` (video stays `wanted`). |
| `cache_beginning` | After stream pack when beginning-cache seconds > 0 | Cache pipe-stream beginning under CacheDir for instant playback start |
...
| `pack_stream` | Pack-stream wanted, Prepare stream, **maturity** for stream series | `.strm` + NFO/sidecars; beginning cache; maturity rebuilds beginning |
| `stream_play` | Stream proxy play (live HLS or beginning/playback cache) | Interactive occupancy via `InsertRunning`; blocks ClaimNext slot; ends on idle TTL (~2m) or Cancel; play yt-dlp pause codes auto soft-pause |
...
| `import` | Import UI | Scan inbox + library orphans; match to indexed videos **or create unmatched**; confirm enqueues **pack** (inbox move) or in-place bind (library) |

Store in `tasks` table. Never use kind name `poll`. Boot renames legacy kind strings and related settings keys (`file_sync` → `sync_files`, `retention_purge` → `retention_delete`, `download_beginning` → `cache_beginning`, …).

### Maintenance outside the domain queue

| Pass | Trigger | Behavior |
|---|---|---|
| **File sync** | `sync_files_cron` (optional) | Online roots only. Cron enqueue is a no-op when there are no videos. (1) `downloaded`/`verify_failed` + path gone → `missing` (keep rows). (2) `missing` + path back → `downloaded`. (3) Streamable beginning cache vs `stream_beginning_cached` (skip if CacheDir offline); beginning loss requeues `cache_beginning` when gates allow. History detail on the finished `sync_files` task when changed (`missing_ids` / `restored_ids` / `beginning_missing_ids` / `beginning_restored_ids`). |
...
| **NFO regenerate** | Settings → Library → Maintenance | System task `regenerate_nfo` with video/series cursors (resumes after Creatorr restart via `RequeueStaleRunning`). Skips write when on-disk bytes match; episode rewrite appends `video_history` (`nfo_regenerated`) with task id. One History outcome on the finished task (`tasks.detail`) on completion. |
| **strm regenerate** | Settings → Library → Maintenance | System task `regenerate_strm` with video cursor. Rewrites `.strm` URL lines to current external URL + token; skips matching bytes. |
| **Clear beginning of stream cache** | Settings → Library → Maintenance | System task `clear_beginning_cache`. Deletes `{CacheDir}/download-beginnings`, clears `stream_beginning_cached`, cancels pending `cache_beginning`. |
| **Clear progressive stream cache** | Settings → Library → Maintenance | System task `clear_playback_cache`. Deletes `{CacheDir}/playback-cache`, zeros progressive cache columns. Leaves beginning caches alone. |
...
- Cooldown between tasks is **per-domain** (`task_cooldown_seconds`; applies after **ClaimNext** / Finish for host lanes, not after interactive prefetch, and never on the `system` lane). While a host domain is cooling down, `/tasks` shows a daisyUI [countdown](https://daisyui.com/components/countdown/#countdown) next to the lane hostname.
- UI **Tasks** page: in-progress + queued tasks grouped by domain (`system` always first, then every known host from domains rows and source URLs, including empty lanes); Pause/Resume per host lane; kind/message link to `/task/{id}`.
...
|---|---|---|
| Scope | Series → its **sources** (tip Scan: series monitored ∧ domain active; full scan: domain active) | Existing **videos** (one or all in series) |
