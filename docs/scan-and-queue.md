...
| `prefetch_add_video` | fetch URL hostname | **Interactive:** Add video modal fetch; draft under `cache/add-video/{token}/` (no video row yet); ignores soft Pause; Finish does not start cooldown |
...
| `import` | Import UI | Scan chosen folder (inbox default, or one library root); match to indexed videos **or create unmatched** via Match modal (name search + poster/thumb lists); confirm enqueues **pack** (inbox move) or in-place bind (library). Create unmatched requires `upload_date` (scan suggests from sidecar or file mtime; API falls back to mtime). Media video list defaults to videos without packed media; **Allow matching existing media** in the modal opts in to replace (API `replace=true` deletes existing library media then imports). ID match order: filename `[id]`, then `info.json` `id`, then NFO `<uniqueid>`; then title / series similarity. Orphan `info.json` is not attachable (provenance with media only). `.strm` is listed only (not importable; regenerate with current External Creatorr URL). Orphan/sibling `.nfo` applies editable episode metadata to the video row then regenerates the library NFO from DB (source XML not kept). `.nfo` is data import only and requires a same-basename video beside it (bundled with media import, or orphan beside packed library media); alone with no sibling video it is listed only. |
...
| **strm regenerate** | Settings → Maintenance | System task `regenerate_strm` with video cursor. Rewrites `.strm` URL lines to current external URL + token; skips matching bytes. |
| **Clear beginning of stream cache** | Settings → Maintenance | System task `clear_beginning_cache`. Deletes `{CacheDir}/download-beginnings`, clears `stream_beginning_cached`, cancels pending `cache_beginning`. |
| **Clear progressive stream cache** | Settings → Maintenance | System task `clear_playback_cache`. Deletes `{CacheDir}/playback-cache`, zeros progressive cache columns. Leaves beginning caches alone. |
