...
Every cancel sets `tasks.message` from a **closed reason code** (no free text; no bare `Cancelled`). Codes live in `internal/queue`; unknown/empty → error (fail closed).

| Code | Message |
|---|---|
| `manual` | `Cancelled (manual)` |
| `shutdown` | `Cancelled (shutdown)` |
| `video_ignored` | `Cancelled (video ignored)` |
| `video_deleted` | `Cancelled (video deleted)` |
...
| `bulk_edit_videos` | `system` | One pending/running; bulk catalog metadata override for selected videos (UI + `POST /api/videos/bulk-metadata`) |
...
| `sponsorblock_cut` | `system` | Per-video dup; appends end of system lane; archive remove/cut + pack after download staging. Not one-per-kind (many may queue). |
| `integrity_check_initial` | `system` | Per-video dup. Null-decode packed library media after pack when profile `verify_media` (mature-only gate when maturity hours set). Import confirm `verify` enqueues only when File integrity is on (still ignores mature timing). Fail → `integrity_check_failed` (keep files). |
...
|---|---|---|
| `feed` (default) | Add feed; full-scan limit + title include/exclude + Scan interval + Mark new videos as ignored | Tip Scan on per-source `scan_cron` / series **Scan** / per-source button |
| `single` | Add single; URL + label | No tip Scan; Full scan OK |

**Mark new videos as ignored** (`index_as_ignored`): when on (**feed** sources only; UI hidden for singles), new videos get status `ignored` instead of `wanted` until manually set to wanted. **Title include / exclude** (`title_regexp_include`, `title_regexp_exclude`): optional Go regexps on **feed** sources (UI hidden for singles). Index only when include matches (if set) and exclude does not match (if set); **exclude wins** when both match (no video row). Tip scan walks past title skips (continue; stop only on known remote id). Empty = no filter. Invalid patterns rejected on save. **Live soft-skip** (always on for download): yt-dlp `is_live` → task `done`, status stays `wanted`, history `live_skipped`, info notify `live_skipped` with task link (retry after broadcast). **Full scan limit** (`full_scan_limit`): optional positive integer on feeds; 0 / empty = unlimited. Full scan only (yt-dlp `--playlist-end`); tip Scan uncapped. Order is extractor list order (usually newest first, not guaranteed).
...
| `download_rate_limit` | `10M` | Passed to yt-dlp as `--limit-rate` for archive download and scan/list. Stored as yt-dlp `K`/`M`/`G` (binary). UI shows `KiB/s` / `MiB/s` / `GiB/s` (or Unlimited → `off`). `off` / `0` / `none` = unlimited. Not used for **interactive** metadata prefetch (`prefetch_*`). |
...
**History (finished scan task):** one finished scan task with detail, e.g. `Scan: indexed 12 videos (3 new, 1 skipped by title include, 1 skipped by title exclude, 1 marked as ignored)` (mode lives in task detail `full` / history `mode`; optional `full_scan_limit`); `created_ids` / `updated_ids`; `skipped_title_regexp_include` / `skipped_title_regexp_exclude` (`[{remote_id, title}, …]`); `ignored_index_as_ignored_ids` (subset of creates; older history may still show `ignored_media_type_ids`). History list shows the task message only (open task detail for video links). Task detail (`/task/{id}`): Detail section lists JSON keys; `created_ids` show id, title, required scan-time state (`wanted`\|`ignored`), and ignore reason when ignored; `skipped_title_regexp_include` / `skipped_title_regexp_exclude` list titles (no video links; ~20 then “and N more”); ignore id arrays render as counts. Not one History row per video.
...
| `sponsorblock_cut` | After archive download when remove categories set | System lane (low priority): remux only if copy-cut; ApplyArchive cut/reencode (one ffmpeg filter_complex; progress 0–1 over output duration); pack; cleanup staging. Cancel wipes staging (like cancel download). |
| `integrity_check_initial` | After successful archive pack when profile gate says so; import confirm `verify` when File integrity on | System lane: ffmpeg null-decode (`-xerror`) of library media with `-progress`, plus hash/NFO when File integrity on. Success → history `integrity_checked` (detail `IntegrityCheckReport`: `outcome` + `checks`). Soft sidecar/NFO → outcome `partial` (status still `downloaded`). Fail → keep files, status `integrity_check_failed`, notify `integrity_check_failed`, code `IntegrityCheckFailed` (detail includes failing check key). Task detail **Results** panel. Cancel leaves `downloaded`. New pack cancels prior verify (superseded). |
| `refresh_sidecars` | Maturity sidecar cron; video detail / series bulk / Settings → Maintenance Run → **Refresh sidecars** | NFO/thumb/subs only beside pack anchor; never media or `info.json`; maturity cron also sets `sidecars_acquired_at`. Maintenance enqueues one domain-lane task per packed video in optional `series_ids` / `video_ids` scope (empty = whole library). |
| `rescan_metadata` | User on video or series; import gap-fill when `source_url` known (`gap_fill: true`) | Update metadata for existing videos only; no new videos. Full rescan rewrites NFO/thumb/subs (not `info.json`). Import `gap_fill` soft-fills empty fields, rewrites NFO from DB, and adds missing thumb/subs only. API: `POST /api/series/{id}/metadata-rescan`, `POST /api/videos/{id}/metadata-rescan` |
| `import` | Import UI | Scan chosen folder (inbox default, or one library root; skips series-folder tvshow.nfo/art); match to indexed videos (suggestion catalog loaded once per scan) **or create unmatched** via Match modal (name search + poster/thumb lists from `GET /api/import/picker`, loaded async after the Import shell paints); confirm enqueues **pack** (inbox move) or in-place bind (library). Create unmatched requires `upload_date` (scan suggests from sidecar or file mtime; API falls back to mtime). Media video list defaults to videos without packed media; **Allow matching existing media** in the modal opts in to replace (API `replace=true` deletes existing library media then imports). ID match order: filename `[id]`, then `info.json` `id`, then NFO `<uniqueid>`; then title similarity; series similarity suggests a series only (no video / no auto-CREATE - operator picks or creates a video in Match). Orphan `info.json` is not attachable (provenance with media only). Sidecar Attach Match only lists videos that already have packed library media (API also rejects attach without media). Inbox subtitle/thumb Attach moves into the episode folder beside media; library orphans must already be beside media. Media import also packs companion `info.json`, `{stem}-thumb` / same-stem images, and subs beside the video. After import, if `source_url` is known, soft-enqueue `rescan_metadata` with `gap_fill: true` (empty episode/NFO fields + missing thumb/subs; import still succeeds if that fails). Orphan/sibling `.nfo` applies editable episode metadata to the video row then regenerates the library NFO from DB (source XML not kept). `.nfo` is data import only and requires a same-basename video beside it (bundled with media import, or orphan beside packed library media); alone with no sibling video it is listed only. |
...
|---|---|---|
| **File sync** | `sync_files_cron` (optional); Settings → Scheduler → Run now; Settings → Maintenance Run → **File sync** (always whole library) | Online roots only. Cron/manual enqueue is a no-op when there are no videos. (1) `downloaded`/`integrity_check_failed` + path gone → `missing` (keep rows). (2) `missing` + path back → `downloaded`. (3) present media: non-NULL `files.size_bytes` vs on-disk size; mismatch → `integrity_check_failed`, update size, history `file_externally_changed` (no auto re-download); NULL size backfilled quietly. (4) Registered sidecars (`kind != video`): missing → keep row + `size_bytes = -1` + `sidecar_missing` (status unchanged); restore + `sidecar_restored`; size mismatch + `sidecar_externally_changed` (status unchanged); NULL size backfilled quietly. Size only - no hash/mtime. History detail when changed (`missing_ids` / `restored_ids` / `externally_changed_ids` / `sidecar_*`). One alert digest `file_sync_issues` when media or sidecar missing/size issues found. |
...
| **NFO regenerate** | Settings → Maintenance Run | System task `regenerate_nfo` with video/series cursors (resumes after Creatorr restart via `RequeueStaleRunning`). Optional Maintenance scope `series_ids` / `video_ids`. Skips write when on-disk bytes match; episode rewrite appends `video_history` (`nfo_regenerated`) with task id. One History outcome on the finished task (`tasks.detail`) on completion. |
| **Integrity check** | Settings → Maintenance Run → **Integrity check** | System task `integrity_check` (one pending/running; resumable cursor). Optional Maintenance scope `series_ids` / `video_ids`. Null-decodes packed `downloaded` / `integrity_check_failed` media (same ffmpeg path as `integrity_check_initial`). Clean ok → history `integrity_checked` + status `downloaded`. Soft sidecar/NFO → `partial` counter + report. Fail → keep files, `integrity_check_failed`, notify `integrity_check_failed`. Skips busy / profile-off / no media with reason counts. Finish detail aggregates check results; task detail **Results** panel. |
| **Bulk edit series** | Series list Edit / Edit metadata (OpenAPI bulk-edit / bulk-metadata) | System task `bulk_edit_series` (one pending/running; 409 when busy). Settings (including monitored) and/or catalog metadata override. |
| **Bulk edit videos** | Series video list Edit metadata (OpenAPI `/api/videos/bulk-metadata`) | System task `bulk_edit_videos` (one pending/running; 409 when busy). Catalog metadata override only; upload date preserved. |
| **File delete** | Series UI delete (required confirm) / video Delete / series list bulk Delete / video list bulk Delete | System task `delete_files` (payload `series_ids` / `video_ids` + cursors). HTTP cancels related work and enqueues; rows stay until worker finishes. Standalone video → `MarkDeleted(..., task_id)`; series purge → remove video artifacts, then **RemoveAll** the series folder (episodes, `tvshow.nfo`, poster/banner/fanart/clearlogo, leftovers), clear `cache/series-meta/{id}/`, then `DELETE series`. UI shows **queued for deletion** from payload membership (no new `videos.status`). REST `DELETE /api/series/{id}` keeps library files. | Per-sidecar Delete (`sub`/`thumb`/`other`): sync `delete_sidecar` bookkeeping + `sidecar_deleted` (not this task). |
| **Apply episode format** | Settings → Maintenance Run | System task `rename_episodes` renames packed episode file sets to current formats. Per-video history `renamed` on success. |

## Queue (per domain)

- Tasks that touch a site belong to a **domain** (from URL hostname).
- One ordered queue per domain: `queue_seq` (append on enqueue; **To top of queue** reassigns). ClaimNext / UI pending order: `queue_seq ASC, id ASC`.
- **Pause** (Tasks lane / `PUT /api/domains/{domain}/paused`): soft stop - no new **ClaimNext** claims; pending stay queued; running continue to Finish. Stored in `domain_runtime` (no `domains` override row). **Immediate** tasks (`ClaimImmediate`: interactive prefetch / probe, and video-page **Download now**) still run while paused (ignore busy/cooldown slots). Cookie-auth, rate-limit, and IP-block (`CookieInvalid` / `RateLimited`) failures **auto soft-pause** that hostname and notify as **alerts** (`cookie_invalid` / `rate_limited`; always recorded in-app). Generic download/resolve failures (`DownloadFailed` / `ResolveFailed`) and remux/pack/verify failures notify as alerts (`ytdlp_failed`) but do **not** auto-pause. Domains are never auto-deactivated. `GET /api/domains/paused` lists soft-paused hosts. Tasks page shows an alert above the lanes summarizing Pause vs interactive; each host lane links to History filtered by that domain.
...
- Cooldown between tasks is **per-domain** (`task_cooldown_seconds`; starts on **ClaimNext** for host lanes, not on Finish, not after immediate claim, and never on the `system` lane). Host lanes can **Skip cooldown** from Tasks (disabled while the lane is paused, even if a cooldown timer is still running). **Process boot** also arms cooldown for every known host (domains override rows + source URL hostnames) before the worker starts claiming, so a restart does not fire downloads immediately (in-memory cooldown is otherwise empty). On `/tasks` each host lane always shows queue status on line 1 left of History: labeled daisyUI [progress](https://daisyui.com/components/progress/) for pause / cooldown / busy (all parallel slots taken, indeterminate), or muted `Idle` when idle; `Active` when running with free parallel slots (indeterminate bar); `Busy` when all parallel slots are taken (indeterminate bar).
- UI **Tasks** page: in-progress + queued tasks grouped by domain (`system` always first, then every known host from domains rows, source URLs, and soft-paused `domain_runtime` hosts, including empty lanes); each lane is one daisyUI task list paged at 10 (`TaskPageSize`, per-host query `p_{host}`); running rows show a static activity icon in the position slot; pending rows show pending-only queue `#N` (1 = next to claim); Pause/Resume per host lane; **To top of queue** always shown (disabled when running or already `#1`); kind/message link to `/task/{id}`.
- **Download now** (video detail only): `ClaimImmediate` (skip queue / parallel / cooldown / soft pause); bypasses `max_download_queue`; promotes an already-pending download; disabled when no source URL, domain inactive, or download already running.
- **Task detail** (`/task/{id}`): any status; live status/message/progress via SSE; in-memory live message + percent (and Logs lines) while running - not written to SQLite on each tick (column `tasks.progress` kept for schema compatibility). On **failed** Finish the Logs ring is written once to `tasks.logs` (cap ~200); done/cancelled clear it. Failed History detail shows that Logs panel without Refresh. Graceful stop (SIGINT/SIGTERM): **pending stay queued**. Running **resumable** kinds stay `running` for boot `RequeueStaleRunning` (not marked `cancelled`). Interactive **prefetch** (`prefetch_series_meta` / `prefetch_video_meta` / `prefetch_add_series` / `prefetch_add_video`) is cancelled with message `Cancelled (shutdown)`. Operator cancel requires closed reason codes (UI/API `manual` → `Cancelled (manual)`); ignore/delete/unmonitor/supersede map to their own messages. One auto Refresh on open for Logs while live, then manual Refresh. Running task detail / Tasks list: mid `(0,1)` determinate bar; 0% / 100% / nil = indeterminate busy. Compact indicators still spinner on nil / 0% / 100%. Download messages label per-format steps and append live yt-dlp throughput when reported (`… 42% · 1.00MiB/s`). Failed tasks show **Error code** (`tasks.error_code`) and **Error** (prefer `tasks.error_message`, else `tasks.detail` JSON `error`, else plain detail) in Details; yt-dlp failures store cleaned ERROR/WARNING lines (progress noise dropped) via MergeDetailJSON without wiping `domain-access` / `po-token`. Host-lane tasks snapshot **Domain access** chips into `tasks.detail` `domain-access` at claim (rate / sleep / cookies / Flare / credentials; same icons as Tasks lane; queue / parallel / cooldown omitted). **Commands** panel lists yt-dlp / ffmpeg / ffprobe argv (shell-quoted) recorded while the task ran: template-deduped (absolute paths → `$PATH` for matching; sample keeps real paths; `×N similar` when repeated), head+tail unique templates (10+10) with an omitted marker, ffprobe capped at 3 distinct templates. Buffered in memory while running; written once to `tasks.commands` on Finish/Cancel for most kinds. Noisy system kinds (`rename_episodes`, `sync_files`, `integrity_check`, `regenerate_nfo`, `retention_delete`, `bulk_edit_*`) persist commands **only on failure** (success/cancel clear the buffer). Pretty toggle (same as JSON file view) switches one-line vs `--`-flag line breaks.
...
- Series detail: per-source combined **Status** (icon + short label: running/pending/error/incomplete/indexed/…). Tooltip covers full-scan limit, next/last scan. Kind icon on the URL row. Remonitor does not auto-enqueue scans.
