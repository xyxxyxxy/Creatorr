...
| `sync_files` | `system` | One pending/running; `sync_files_cron` or Scheduler Run now; higher priority than Apply |
...
**Tip scan** indexes new `wanted` videos only (no immediate download enqueue). Full scan stays index-only. Wanted videos wait for `download_wanted_cron` / **Queue download**.
