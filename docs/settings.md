...
| Key | Role |
|---|---|
| `pot_fetch` | **PO token fetch** (PO = proof-of-origin): `auto` (default) / `always` / `never` → yt-dlp `youtube:fetch_pot`. Settings → General. Control disabled (label shows `(disabled)`, value forced to `never`) until env `CREATORR_POT_PROVIDER_URL` is set. Compose default `http://creatorr-po-token:4416`; empty `.env` value disables (Compose uses `${VAR-default}`, not `:-`). When URL unset, runtime and Save also force `never`. |
