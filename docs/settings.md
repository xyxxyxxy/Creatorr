...
| `metadata_domain_tag` | `1` = prepend source domain to video tags on download, stream pack, and Metadata Save when `source_url` is known (default on). Locked in the video Metadata Tags editor. Settings → Library → Metadata. Not retroactive until next pack or Save. |
| `metadata_genres_from_categories` | `1` = add yt-dlp categories as video genres on download, stream pack, and Metadata Save when categories are known (default on). Locked rows in the video Metadata Genres editor. Settings → Library → Metadata. Not retroactive until next pack or Save. |
...
**Domain limits:** stored on `domains` row `domain=default` (non-NULL cooldown, max download queue, max parallel tasks, download rate, stream play rate, sleep, Use FlareSolverr; optional cookies and optional membership **credentials** username/password). Host overrides are other `domains` rows (NULL = inherit). Credentials are separate from cookies: jars hold Cloudflare clearance; username/password feed yt-dlp `--username` / `--password` for site plugins that perform membership login (tokens may cache in yt-dlp's default `--cache-dir`, not Creatorr-managed). Soft **Pause** is separate (`domain_runtime`; Tasks / API). Settings → Queue / Domains (defaults form + overrides list without Active column + shared Edit modal). Hostname on Add/Edit must look like a DNS name (rejects `example,com`, bare labels, URLs). Changing hostname on Edit upserts the posted host only (previous override row left as-is; overrides are soft lookups by hostname). Settings → Domains redirects to Queue / Domains. No auto-create on source add. `max_parallel_tasks` must be ≤ `max_download_queue`. Pace: `download_rate_limit` + `sleep_requests` for archive/scan/`cache_beginning`; `stream_play_rate_limit` for live play mux/pipe only (default Unlimited; no sleep on play). Interactive metadata prefetch (`prefetch_*`) ignores rate and sleep.
...
| Quality profiles | `best` (format `bv*+ba/b`, default), `1080p` (`bv*[height<=1080]+ba/b[height<=1080]/bv*+ba/b`), `720p` (`bv*[height<=720]+ba/b[height<=720]/bv*+ba/b`). Height profiles keep soft unrestricted tails. Bare yt-dlp `best` alone is avoided as the primary selector (soft progressive on DASH sites). Remux is always MKV. |
