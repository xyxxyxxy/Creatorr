...
- **Cookies after download failure** (`domains.cookies_after_fail`, override Access checkbox **Use cookies only after first download failure**, default off): when on, the stored jar is omitted for scan / metadata / sidecar yt-dlp calls. Downloads try without the jar first, then retry once with cookies unless the failure is clearly cookie-irrelevant (`RateLimited`, `LiveBroadcastSkipped`, removed/unavailable video, cancel). Soft-pause / alerts use the **final** attempt only. Task detail stores `cookie-attach` (`anonymous` / `retried` / `cookies` / …); Stages show a `cookies` node; Details show **Account cookies**. Leave unchecked when the jar must be on every request (e.g. clearance paste).
