...
- Membership credentials on the same `domains` rows (`username` / `password`; NULL inherit on host overrides): passed as yt-dlp `--username` / `--password` when non-empty. Site plugins may cache access tokens in yt-dlp's default cache (`~/.cache/yt-dlp` under process `HOME`; Creatorr does not pass `--cache-dir`). Do not export member session cookies into the jar when a plugin supports login.
