...
| Detect | yt-dlp output is scanned for provider failures (`Providers: none`, HTTP ping/mint errors), mint start (`Generating a … PO Token` → live `generating`), and successful mints (`Retrieved a … PO Token` → `issued`). When the task ends still on `generating` (mint started, never Retrieved), Creatorr finalizes to `skipped`. Cookie-retry downloads store per-invoke outcomes in `po-token.attempts` so Stages can show each pass separately. The task still succeeds on provider problems; Creatorr emits warning notification `pot_provider` (unread like alerts). Outcome is stored on the task as detail JSON `po-token` (`issued` / `generating` while live / `failed` / `skipped` / `off`) shown on the task Details row **PO token**, and nested under download Stages as **PO used** / **PO skipped** (also failed; live may show generating; `off` shows as skipped). Live task detail refreshes that row via HTMX while the task runs. |
...
- **Cookies after download failure** (`domains.cookies_after_fail`, override Access checkbox **Use cookies only as fallback for failed downloads**, default off): when on, the stored jar is omitted for scan / metadata / sidecar yt-dlp calls. Downloads try without the jar first, then retry once with cookies unless the failure is clearly cookie-irrelevant (`RateLimited`, `LiveBroadcastSkipped`, removed/unavailable video, cancel). Soft-pause / alerts use the **final** attempt only. Task detail stores `cookie-attach` (`anonymous` / `retried` / `cookies` / …); Stages show two download nodes when a cookie retry ran; **Cookies used** when the jar was attached (any mode); **PO used** / **PO skipped** on download nodes from task-level `po-token`; Details show **Account cookies**. Leave unchecked when the jar must be on every request (e.g. clearance paste).
